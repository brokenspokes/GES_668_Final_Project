...
st_read("Data/neighborhood_stats.gpkg") |>
  ...
  pivot_longer(cols = ends_with("_n"),
               names_to = "Sale_Type",
               values_to = "N_Sales") |>
  mutate(Sale_Type = gsub("_n", "", Sale_Type)) |>
  filter(Sale_Type != "regular")-> pivot_no_reg

noreg_sale_dot_density <- as_dot_density(pivot_no_reg,
                                   value = "N_Sales",
                                   values_per_dot = 1,
                                   group = "Sale_Type")

rdeck_dark_noreg <- rdeck(map_style = mapbox_dark(),
                    theme = "kepler",
                    initial_bounds = st_bbox(pivot),
                    height = 600
)

noreg_sale_dot_density |>
  st_transform(4326) |>
  st_set_geometry("position") -> noreg_sale_dot_density

rdeck_dark |>
  add_scatterplot_layer(
    name = "Sales by type of property (1 per dot)",
    data = noreg_sale_dot_density,
    radius_min_pixels = 1,
    radius_max_pixels = 5,
    radius_scale = 5,
    opacity = 0.6,
